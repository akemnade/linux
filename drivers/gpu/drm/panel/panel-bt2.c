...
MODULE_PARM_DESC(msdelay, "random wait time");
...
static int tc358762_write_register32(struct bt200 *ctx, u16 addr, u32 val)
{
	return tc358762_write_register(ctx, addr, val, sizeof(u32));
}
...
static int bt200_unprepare(struct drm_panel *panel)
{
	...
	tc358762_write_register32(ctx, LCDCTRL0, 0);
...
static int init_lcd(struct bt200 *ctx)
{
	int i;
	int r;
	/* this does what? configures CS, asserts it or both */
...
static int init_seq(struct bt200 *ctx)
{
	...
	u32 rback;
	u32 syspll;
	tc358762_write_register32(ctx, RDPKTLN, 3);
	tc358762_write_register32(ctx, DSI_LANEENABLE, 7); /* not needed for reads work, but display stays white */
	tc358762_write_register32(ctx, PPI_D0S_CLRSIPOCOUNT, 0x00000003); /* not needed for reads work but display stays wihte */
	tc358762_write_register32(ctx, PPI_D1S_CLRSIPOCOUNT, 0x00000003); /* not needed for reads work but display stays white */
	tc358762_write_register32(ctx, PPI_D0S_ATMR, 0x00000000); /* not needed for reads work but display stays white */
	tc358762_write_register32(ctx, PPI_D1S_ATMR, 0x00000000); /* net needed for reads work but display stays white */
	tc358762_write_register32(ctx, PPI_LPTXTIMECNT,7);

	tc358762_write_register32(ctx, SPITCR1, 0x00000122); /* can be done after resetting STARTPPI to 0 */
	tc358762_write_register32(ctx, SYSCTRL, 0x0000020F);  /* no correct picture on right, stays white, left is distorted */

	tc358762_write_register(ctx, PPI_STARTPPI, 1, sizeof(u32));
	/* fails at >2nd prepare call */
	dev_info(ctx->dev, "id reg %d %x\n" ,tc358762_read_register(ctx, IDREG, &rback), rback);
	/* and these are then still working */
	dev_info(ctx->dev, "id reg %d %x\n" ,tc358762_read_register(ctx, IDREG, &rback), rback);
	dev_info(ctx->dev, "id reg %d %x\n" ,tc358762_read_register(ctx, IDREG, &rback), rback);
	// Rx read

	/* 3ff0000 after POR on left, 3ff00c0 on right, after reset via GPIO15: both 3ff00c0 */
	dev_info(ctx->dev, "spictrl  %d %x\n" ,tc358762_read_register(ctx, SPICTRL, &rback), rback);
	/* right display stays white without this */
	//tc358762_write_register32(ctx, PPI_STARTPPI, 0);
	tc358762_write_register32(ctx, LCDCTRL0, 0x00000152);
	tc358762_write_register32(ctx, HSR, 0x00560014);
	tc358762_write_register32(ctx, HDISPR, 0x002103C0);	// HFP=33
	tc358762_write_register32(ctx, VSR, 0x000C0003);
	tc358762_write_register32(ctx, VDISPR, 0x0009021C);
	tc358762_write_register32(ctx, VFUEN, 0x00000001);

	msleep(100);
	r = tc358762_write_register(ctx, PPI_STARTPPI, 1, sizeof(u32));
	r = tc358762_write_register(ctx, DSI_STARTDSI, 1, sizeof(u32));
	syspll = 0;
	dev_info(ctx->dev, "id reg %d %x\n" ,tc358762_read_register(ctx, IDREG, &rback), rback);
	dev_info(ctx->dev, "id reg %d %x\n" ,tc358762_read_register(ctx, IDREG, &rback), rback);
	...
	/* 
	 * trigger a CIO off, vendor kernel does that after SYSPLL3 write,
	 * full initialisation will work after 2nd prepare()
	 */
	if (r)
		return r;
...
static int bt200_enable(struct drm_panel *panel)
{
	...
	dev_dbg(ctx->dev, "%s\n", __func__);
	dsi->mode_flags &= ~ MIPI_DSI_MODE_LPM;
	r = tc358762_write_lcd(ctx, 0x0A, 1 );
	//msleep(5);
	//msleep(100);
	return r;
