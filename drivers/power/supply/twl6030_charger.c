 ...
 */
static int twl6030_clear_set(u8 mod_no, u8 clear, u8 set, u8 reg)
{
	u8 val = 0;
	int ret;
...
static void twl6030_config_cinlimit_reg(struct twl6030_charger_info *charger,
				        unsigned int ua)
{
        int ret;
        ...
        if ((ua >= 50000) && (ua <= 750000))
		 ...
		 (device_is_compatible(charger->dev, "ti,twl6032-charger"))) {
                        ua = ((ua % 100000) ? 0x30 : 0x20) +
                                                ((ua - 100000) / 100000);
        } else if (ua < 50) {
                dev_dbg(charger->dev, "invalid input current limit\n");
                return;
        } else {
        ...
        ret = twl6030_charger_write(CHARGERUSB_CINLIMIT, ua);
        if (ret)
 ...
 */
static void twl6030_enable_usb(struct twl6030_charger_info *charger)
{
	twl6030_charger_write(CHARGERUSB_VICHRG,
	...
	twl6030_config_cinlimit_reg(charger, charger->input_current_limit);

	twl6030_charger_write(CHARGERUSB_CINLIMIT, CHARGERUSB_CIN_LIMIT_500);

	twl6030_charger_write(CHARGERUSB_VOREG,
		UV_TO_VOREG(charger->binfo->constant_charge_voltage_max_uv));

	twl6030_charger_write(CHARGERUSB_CTRL1, TERM);
	...
	twl6030_charger_write(CONTROLLER_CTRL1, CONTROLLER_CTRL1_EN_CHARGER);
}
...
static void twl6030_charger_wdg(struct work_struct *data)
{
	...
	dev_dbg(charger->dev, "wdg: stat1: %02x %s INT_STATUS %02x STATUS_INT1 %02x STATUS_INT2 %02x\n", val,
		(val & VBUS_DET) ? "usb online":  "usb offline", int_stat, stat_int1, stat_int2);
...
static irqreturn_t twl6030_charger_interrupt(int irq, void *arg)
{
	...
	dev_dbg(charger->dev, "charger irq: stat1: %02x %s INT_STATUS %02x STATUS_INT1 %02x STATUS_INT2 %02x\n", val,
		(val & VBUS_DET) ? "usb online":  "usb offline", int_stat, stat_int1, stat_int2);
	power_supply_changed(charger->usb);

	if (val & VBUS_DET) {
		twl6030_charger_read(CONTROLLER_CTRL1, &val);
		if (!(val & CONTROLLER_CTRL1_EN_CHARGER)) {
			twl6030_enable_usb(charger);
			schedule_delayed_work(&charger->charger_monitor,
					      msecs_to_jiffies(10000));
...
static int twl6030_charger_probe(struct platform_device *pdev)
{
	...
	if ((charger->binfo->constant_charge_voltage_max_uv > 4760000) ||
	    (charger->binfo->constant_charge_voltage_max_uv < 3500000)) {
	...
	ret = devm_request_threaded_irq(&pdev->dev, charger->irq_chg, NULL,
			twl6030_charger_interrupt, IRQF_ONESHOT, pdev->name,
			charger);
	if (ret < 0) {
		return dev_err_probe(&pdev->dev, ret,
